...
if __name__ == '__main__':
    ...
    features_mat = feature_scaler.fit_transform(features_mat)
    ...
    gp = GaussianProcessRegressor(kernel = RationalQuadratic(alpha = 500, length_scale = 3), normalize_y = True)
    gp.fit(train_features, train_labels[:,np.newaxis])
    predictions = gp.predict(test_features)
